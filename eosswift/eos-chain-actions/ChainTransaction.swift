    ...
    func push(
        ...
        return chainApi().getInfo().flatMap { info in
            if (info.success) {
                ...
                // JSON
                let rrr = PushTransaction(signatures:                               [signature],
                                          compression:                              "none",
                ...
                return self.chainApi()
                            .pushTransaction(body: PushTransaction(signatures:                  [signature],
                                                                   compression:                 "none",
                                                                   packed_context_free_data:    "",
                                                                   packed_trx:                  transactionAbi.toHex()
                            ))
                            .map { response in
                                ChainResponse<TransactionCommitted>(success:        response.success,
                                                                    statusCode:     response.statusCode,
                                                                    body:           response.body)
                            }
                            .catchError { error in
                                let errorResponse = (error as! HttpErrorResponse<ChainError>)
                                return Single.just(ChainResponse<TransactionCommitted>(success:     false,
